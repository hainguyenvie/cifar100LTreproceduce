...
CONFIG = {
    ...
    'aurc_eval': {
        ...
        'use_full_test_set': False,  # Set to False for proper validation methodology
    },
...
def compute_aurc_from_points(rc_points):
    ...
    # Trapezoidal integration (using scipy to avoid deprecation warning)
    try:
        from scipy import integrate
        aurc = integrate.trapezoid(risks, coverages)
    except ImportError:
        # Fallback to numpy if scipy not available
        aurc = np.trapz(risks, coverages)
    return aurc

# Note: evaluate_aurc_comprehensive function was removed to avoid code duplication
# The AURC evaluation logic is now directly integrated in main() function
...
def main():
    ...
    # Apply per-group thresholds based on PREDICTED groups (realistic deployment)
    t_group = checkpoint.get('t_group', None)
    if t_group is not None:
        # Convert to tensor if it's a list
        if isinstance(t_group, list):
            t_group_tensor = torch.tensor(t_group, dtype=margins_raw.dtype)
        else:
            t_group_tensor = t_group.to(margins_raw.dtype)
        
        # Use PREDICTED groups (realistic deployment scenario)
        pred_groups = class_to_group_cpu[preds]              # [N]
        thresholds_per_sample = t_group_tensor[pred_groups]  # [N]
        accepted = margins_raw >= thresholds_per_sample
        
        print(f"✅ Using per-group thresholds based on PREDICTED groups: {[f'{t:.3f}' for t in t_group_tensor.tolist()]}")
        print(f"✅ Test coverage (inference-realistic): {accepted.float().mean():.3f}")
        
        # Report coverage by PREDICTED groups (router analysis)
        print("   Router behavior (coverage by predicted groups):")
        for k in range(len(t_group_tensor)):
            mk = (pred_groups == k)
            if mk.any():
                cov_k = accepted[mk].float().mean().item()
                gname = "head" if k == 0 else "tail"
                print(f"   📊 {gname} predictions (pred k={k}): coverage={cov_k:.3f}, t_k={t_group_tensor[k].item():.3f}")
        
        # Report coverage by TRUE groups for analysis
        print("   Ground-truth analysis (coverage by true groups):")
        true_groups = class_to_group_cpu[test_labels]
        for k in range(len(t_group_tensor)):
            mk = (true_groups == k)
            if mk.any():
                cov_k = accepted[mk].float().mean().item()
                gname = "head" if k == 0 else "tail"
                print(f"   📊 {gname} true (true k={k}): coverage={cov_k:.3f}, t_k={t_group_tensor[k].item():.3f}")
    else:
    ...
    # Create normalized margin z = m_raw - t_{g(pred)} for proper RC/AURC evaluation
    if t_group is not None:
        pred_groups = class_to_group_cpu[preds]
        t_group_tensor = t_group_tensor if isinstance(t_group_tensor, torch.Tensor) else torch.tensor(t_group, dtype=margins_raw.dtype)
        z_margin = margins_raw - t_group_tensor[pred_groups]
        print("✅ Using normalized margins z = m_raw - t_{g(pred)} for RC/AURC curves")
    else:
        # If no per-group threshold, z == m_raw
        z_margin = margins_raw
        print("✅ Using raw margins for RC/AURC curves (global threshold)")
    
    # 5. Calculate metrics
    print("📈 Calculating metrics...")
    results = {}
    
    # 5.1 RC Curve and AURC (using normalized margins z for proper per-group policy)
    rc_df = generate_rc_curve(z_margin, preds, test_labels, class_to_group_cpu, num_groups)
    ...
    rc_df_02 = generate_rc_curve_from_02(z_margin, preds, test_labels, class_to_group_cpu, num_groups)
    ...
    mean_aurc, lower, upper = bootstrap_ci(
        (z_margin, preds, test_labels), aurc_metric_func, n_bootstraps=CONFIG['eval_params']['bootstrap_n']
    )
    ...
    # 5.4 Metrics at fixed coverage points (using normalized margins z)
    results_at_coverage = {}
    for cov_target in CONFIG['eval_params']['coverage_points']:
        thr_cov = torch.quantile(z_margin, 1.0 - cov_target)
        accepted_mask = z_margin >= thr_cov   # >= giúp bền vững khi có ties
    ...
    # Use normalized margins z for proper per-group rejector evaluation
    gse_margins = z_margin
    
    if CONFIG['aurc_eval']['use_full_test_set']:
        # Use full test set without split (for direct comparison with traditional method)
        print("📊 Using full test set for AURC evaluation (no val/test split)")
        gse_margins_val = gse_margins
        preds_val = preds  
        labels_val = test_labels
        
        gse_margins_test = gse_margins
        preds_test = preds
        labels_test = test_labels
        
        print(f"📊 AURC Data - Full test set: {len(test_labels)} samples")
    else:
    ...
    print("="*60)
    if t_group is not None:
        print("📊 Per-group rejector evaluation:")
        print("   • Thresholds: Applied based on PREDICTED groups (realistic deployment)")
        print("   • RC/AURC curves: Use normalized margins z = m_raw - t_{g(pred)}")
        print("   • Policy: accept(x) ⟺ m_raw(x) - t_{g(ŷ(x))} ≥ δ")
        print("   • This reflects the actual per-group rejector learned during training")
    else:
        print("📊 Global rejector evaluation:")
        print("   • Single threshold applied uniformly")
        print("   • RC/AURC curves: Use raw margins m_raw")
    
    print("\n📊 Traditional RC vs. Comprehensive AURC:")
    print("   • Traditional: Fixed RC curve generation")
    print("   • Comprehensive: Sweeps cost values c with optimal threshold selection")
    if CONFIG['aurc_eval']['use_full_test_set']:
        print("   • Both use full test set evaluation")
    else:
        print("   • Comprehensive: Proper val/test split for unbiased evaluation")
    print("\n💡 Both methods now use the same margin type for fair comparison!")
    print("   The comprehensive method follows academic best practices for AURC.")
    ...
    print()
    print("Key Results:")
    margin_type = "normalized z = m_raw - t_{g(pred)}" if t_group is not None else "raw margins"
    print(f"📊 Traditional RC Metrics (using {margin_type}):")
    print(f"• AURC (Balanced): {aurc_bal:.4f}")
    print(f"• AURC (Worst): {aurc_wst:.4f}") 
    
    print(f"\n🎯 Comprehensive AURC (using {margin_type}, 'Learning to Reject' methodology):")
